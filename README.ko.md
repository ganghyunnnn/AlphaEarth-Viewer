...
- **지명·좌표 검색** — 지명(OpenStreetMap / Nominatim) 또는 `위도, 경도` 입력으로 이동. 🔍 아이콘으로 접기 가능.
- **베이스맵 전환** — 위성(Esri, 기본) / 다크 / OSM. 비교 모드의 양쪽 창에 동일하게 적용.
- **임베딩 투명도·on/off** — AlphaEarth 레이어 투명도 조절, 또는 완전히 꺼서 베이스맵만 보기.
- **대비 값 키보드 입력** — min/max rescale 값을 직접 타이핑(슬라이더와 양방향 동기화).
